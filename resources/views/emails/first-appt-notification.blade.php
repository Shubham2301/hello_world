<div style="width:100%;">
	<div style="max-width:80%;width: 98%;display: inline-block;padding: 1%;font-family: arial, sans-serif;border: solid 1px transparent;border-radius: 2px;margin: 1% 10%;background-color: white;box-shadow: 0 0 5px #ddd;">
		...
		<div>
		    @if(trim($appt['provider_name']) != '')
			<div style="width: 90%;display: inline-block;margin:1em 5%;">
				<p style="font-size: 130%;color:#333">{{ $appt['provider_name'] }}</p>
			</div>
			@endif
		</div>
		<div>
			<div style="width: 90%;display: inline-block;margin:0 5%;text-align:center;">
				<p style="font-size: 120%;color:#4d4d4d">{{ $appt['practice_name'] }} received following appointment for the first time
				@if(trim($appt['provider_name']) != '')
                    with <br> 
                    <span style="font-weight:bold">{{ $appt['provider_name'] }}</span>
				@endif
				@if(trim($appt['location_name']) != '')
                    <br>at <span style="font-weight:bold">{{ $appt['location_name'] }}</span>
				@endif
				@if(trim($appt['location_address']) != '')  
				    ({{ $appt['location_address'] }})
				@endif
				</p>
				<hr style="border: solid 1px #ddd;">
                <p style="font-size: 120%;color:#4d4d4d">This appointment request was generated by  <br> <span style="font-weight:bold">{{ $appt['user_name'] }}</span>, at <span style="font-weight:bold">{{ $appt['user_network'] }}</span> via the <strong>illuma</strong> Platform. </p>
				<p style="font-size: 120%;color:#4d4d4d"> For questions about this appointment please call <br> <span style="font-weight:bold">{{ ($appt['user_phone'] == null || $appt['user_phone'] == '' ) ? $appt['practice_phone'] : $appt['user_phone'] }}</span> or email at <span style="font-weight:bold">{{ $appt['user_email'] }}</span>. </p>
				<br>
