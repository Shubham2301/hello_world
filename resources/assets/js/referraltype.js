...
function printList() {
    ...
    info[0].forEach(function (referraltype) {
        content += '<div class="referral_type"><span class="glyphicon glyphicon-remove-circle remove_referral_type" data-id="' + referraltype.id + '"></span><div class="tile" id="openModel" data-toggle="modal" data-target="#' + referraltype.id + '">                <center><span class="tile_bar"></span><br><p class="tile_name">' + referraltype.name + '</p></center></div>' + referraltype.display_name + '</div><div id="' + referraltype.id + '" class="modal fade" role="dialog" data-id="' + referraltype.id + '"><div class="modal-dialog modal-style" style="color:#000;padding:50px;margin-top:35vh;background-color:#f2f2f2;"><p>Clinical Protocol for scheduling patients for ' + referraltype.display_name + '</p><p>This is consits of 3 stages.</p><div class="modal-footer" style="text-align:center;"><button type="button" class="btn btn-primary referr_patient" data-id="' + referraltype.id + '">Confirm</button><button type="button" class="btn dismiss_button" data-dismiss="modal">Cancel</button></div></div></div>';
    });
